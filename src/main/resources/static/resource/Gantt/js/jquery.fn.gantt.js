...
(function ($, undefined) {
    ...
    $.fn.gantt = function (options) {
        ...
        var settings = {
            ...
            onRightClick: function(e,data){return;},
                            ...
                            if (rday.getMonth() !== month) {
                                ...
                                monthArr.push(
                                    ('<div class="row header month" style="width:'
                                       ...
                                       + 'px;"><div class="fn-label">'
                                       + settings.months[month]
                                       + ' <input type="number"   hidden value='+settings.curMonth+'></div></div>'));
                                month = rday.getMonth();
                                daysInMonth = 0;
                            }
                            daysInMonth++;
                       ...
                       // .append($('<button type="button" class="nav-link nav-prev-week"/>')
                        //   .html('&lt;&lt;')
                          //  .click(function () {
                              //  core.navigateTo(element, tools.getCellSize() * 7);
                            //}))
                        .append($('<button type="button" class="nav-prev-month"/>')
                            .html('上月')
                            .click(function () {
                             var cur_year_next=parseInt($("#curyear").val());
                             var cur_month_next=parseInt($("#month").val());
                             if(cur_month_next==1){
                             	 cur_year_next=cur_year_next-1;
                             	cur_month_next=12;
                             }
                             else{
                             	cur_month_next=parseInt($("#month").val()-1);
                             }
                             if(cur_month_next < 10){
                            	 cur_month_next="0"+cur_month_next;
               				 }
	                         initview(cur_year_next, cur_month_next);
                            }))
                       .append($('<button type="button" class="nav-link nav-now"/>')
                           .html('&#9679;')
                           .click(function () {
                                 var myDate = new Date();
                                 var mmonth=parseInt(myDate.getMonth())+1;
                            	 if(mmonth < 10){
                   					mmonth="0"+mmonth;
                   				 }
	    						 initview(myDate.getFullYear(),mmonth);
                           }))
                        .append($('<button type="button"  />')
                            .html('下月')
                            .click(function () {
              				   var yyear=$("#curyear").val();
              				   var mmonth=parseInt($("#month").val())+1;
//              				 if(mmonth==2){
//                             	yyear=parseInt(yyear)+1;
//                             }
//              				 if(mmonth==13){
//                              	yyear=parseInt(yyear)+1;
//                              }
...
//                              }
            ...
            createProgressBar: function (days, cls, desc, label, dataObj) {
                ...
                var barMarg = tools.getProgressBarMargin() || 0;
                 var barvalue="";
                if(dataObj.cls!=null){
                	barvalue=dataObj.cls;
                }
                var bar = $('<div class="bar" name="bar"><div class="fn-label">' + barvalue + '</div></div>')
                        .css({
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                default:
									...
									//var top = tools.getCellSize() * 4 + 2 + parseInt(topEl.attr("offset"), 10); css top -2
