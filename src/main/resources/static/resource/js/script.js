var App = function () {
	...
	jQuery('.sidebar-menu .has-sub .sub .has-sub-sub > a').click(function () {
		...
		// Handle sub-sub menus
		jQuery('.sidebar-menu .has-sub .sub .has-sub-sub .sub-sub >li').click(function () {
            var last = jQuery('.has-sub-sub.open', $('.sidebar-menu'));
            var self = $(this).parent().parent();
            if(self.text()!=last.text()){
            	last.removeClass("open");
                jQuery('.arrow', last).removeClass("open");
                //jQuery('.sub-sub', last).hide();
                
                self.addClass("open");
                jQuery('.arrow', self).addClass("open");
            }
        });
		
		// Handle sub-sub menus
		jQuery('.sidebar-menu .has-sub .sub > li').click(function () {
            var last = jQuery('.has-sub.open', $('.sidebar-menu'));
            var self = $(this).parent().parent();
            if(self.text()!=last.text()){
            	last.removeClass("open");
                jQuery('.arrow', last).removeClass("open");
                jQuery('.sub', last).hide();
                
                self.addClass("open");
                jQuery('.arrow', self).addClass("open");
            }
            if(!$(this).hasClass("has-sub-sub")){
            	last = jQuery('.has-sub-sub.open', $('.sidebar-menu'));
                last.removeClass("open");
                jQuery('.arrow', last).removeClass("open");
                jQuery('.sub-sub', last).hide();
            }
	...
	var handleBackstretch = function () {
		 $.backstretch([
		"resource/img/login/1.jpg"
		, "resource/img/login/2.jpg"
		, "resource/img/login/3.jpg"
		, "resource/img/login/4.jpg"
