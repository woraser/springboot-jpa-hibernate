...
(function ($, undefined) {
    ...
    $.fn.gantt = function (options) {
        ...
        var settings = {
			...
			onRightClick: function(e,data){},
			onItemRightClick:function(e,data){},
        ...
        var core = {
            ...
            leftPanel: function (element) {
                ...
                var ganttLeftPanel = $('<div class="leftPanel"/>')
                    ...
                    .css("height", tools.getHeaderSize() + "px")
                    .css("width", "100%"));
                        ...
                        for (var i = 0, len = range.length; i < len; i++) {
                            ...
                            if (rm !== month) {
                                monthArr.push(
                                    ...
                                    + tools.getCellSize() * daysInMonth + 'px" id=1><div class="fn-label">'
                        ...
                        .append($('<div class="nav-slider" />')
                            .append($('<div class="nav-slider-left" />')
                                .append($('<button type="button" class="nav-link nav-prev-day"/>')
                                    .html('&lt;')
                                    .click(function () {
                                      	  var timestr=$("#mindates").val();
                                    	  var timearr=timestr.split("-");
                                    	  
                                    	  initGannt(timearr[0],timearr[1],timearr[2],"pre","no");
                                    })))
                            .append($('<div class="nav-slider-right" />')
                                .append($('<button type="button" class="nav-link nav-next-day"/>')
                                    .html('&gt;')
                                    .click(function () {
                                    	  var timestr=$("#maxdates").val();
                                    	  var timearr=timestr.split("-");
                                    	  initGannt(timearr[0],timearr[1],parseInt(timearr[2])+1,"next","no");
                                    }))
                                    )
                                );
            ...
            // the entire chart
            createProgressBar: function (days, cls, desc, label, dataObj, parent) {
		        // dl,
                // day.customClass ? day.customClass : "",
                // day.desc ? day.desc : "",
                // day.label ? day.label : "",
                // day//day.dataObj ? day.dataObj : null
                ...
                $.each(dataObj.scheTimes, function (j, labelvalues) {
                		initPx = (30)*j+2;   
                		
                		if(labelvalues.serviceType==null){
                			labelvalues.serviceType="";
                		}
                      str+='<div class="ui-draggable ui-draggable-handle wodiv pop" wolId="'+labelvalues.wolId+'" data-content="'+labelvalues.description+'" laborer="'+dataObj.laborerId+'"  woid="'+labelvalues.woId+'"  style="position: absolute; '+
                		'left: 2px; top: '+initPx+'px; width: 90px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis; height: 28px; padding: 0px;border-radius:4px;background-color: '
                		+labelvalues.priorityColor+';"data-title="'+labelvalues.woContent+'&nbsp;&nbsp;'+labelvalues.serviceType+'" >'+
						'<span>'+labelvalues.woContent+'</span>'
						+'</div> ';
                      if( (initPx+30) >= parent.height()){
							var rowheader_id = "#rowheader" + parent.attr('id').substr(3);
							var rowheader = $(rowheader_id);
							rowheader.css("height",rowheader.height()+30+"px");
							parent.css("height",parent.height()+30+"px");
				         }
                      //ui.draggable.css("top",(length*(height+2) + 2)+"px");
                      //initPx=initPx+30; 
                });
                      
                // var str='<div class="draggable ui-draggable ui-draggable-handle" style="position: absolute; '+
                // 'left: 2px; top: 2px; width: 80px; height: 28px; padding: 0px;">'+
				// '<span>工单555</span>'+
				// +'</div> ' ;
                var bar = $('<div class="bar" time="'+dataObj.from+'" laborer="'+dataObj.laborerId+'" shcid="'+dataObj.sche_id+'"   ><div class="fn-label" >' + str +label+ '</div></div>')
                        .css({
                            width: ((cellWidth * days) - barMarg) - 1,
				...
				$('.pop').popover({ trigger: 'hover' });
                    ...
                    bar
                      ...
                      .mouseout(function () {
                    		$("#rMenu_assignment").removeClass("open");
                      })
                      .mousemove(function (e) {
                      });
                
                var alreadyDivs=bar.find(".wodiv");
		        $.each(alreadyDivs, function (j, divvalues) {
		   		   divvalues.onmousedown= function(e) {
		   		   		 if(3 == e.which){
		   		   			 dataObj["woid"]=$(divvalues).attr('woid');
		   		 		 }
		   		 		 $(this).data("dataObj", dataObj);
		   		 		 settings.onItemRightClick(e,$(this));
		    	    };
				});
				...
				bar.mousedown(function(e){
				 if(3 == e.which){
					...
					settings.onRightClick(e,$(this));
				});
				
                bar.click(function (e) {
                    e.stopPropagation();
					var col = core.elementFromPoint(e.pageX, e.pageY + 72);
            ...
            fillData: function (element, datapanel, leftpanel /* <- never used? */) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
						...
						row.addClass("paiban");
						row.css("height",tools.getBarHeight());
						row.css("width","100%");
						row.css("float","left");
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                default:
									...
									//var top = tools.getHeaderSize() * 4 + 2 + parseInt(topEl.attr("offset"), 10); css top -2
									var top = tools.getHeaderSize() + parseInt(topEl.attr("offset"), 10);
